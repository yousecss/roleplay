...
module.exports = {
  ...
  çalıştırılıyor(message, args) {
    ...
    if (args.length < 3) {
      return message.channel.send('Kullanım: !tf-liste @TakımRolü <Oyuncuİsmi> <Para>');
    }
...
const formattedPara = para.toLocaleString('tr-TR'); 
    ...
    const logKanalı = message.guild.channels.cache.find(channel => channel.id === '1143215387501142219');
    if (logKanalı) {
      const embed = new MessageEmbed()
        .setColor('#0099ff')
        .setTitle(`${mentionedRole.name} Takımı Transfer Listesi`)
        .setDescription(`${mentionedRole.name} takımı **${oyuncuİsmi}** oyuncusunu **€${formattedPara}** maliyetine transfer listesine ekledi.`)
        .setTimestamp();
      logKanalı.send({ embeds: [embed] });
    }

    message.channel.send('Transfer listesine oyuncu eklendi. <#1143215387501142219>');
  },
};
